...
def process_new_binary_file(new_file):
	...
	h_base_path = "{0}/{1}".format(os.getenv('HUSER','/user/duxbury'), ingest_type)
	...
	#load to avro
	load_avro_cmd = "hive -hiveconf dbname={6} -hiveconf y={0} -hiveconf m={1} -hiveconf d={2} -hiveconf h={3} -hiveconf data_location='{4}' -f oni/load_{5}_avro_parquet.hql".format(binary_year,binary_month,binary_day,binary_hour,h_stage_path,ingest_type,os.getenv('DBNAME','duxbury') )
